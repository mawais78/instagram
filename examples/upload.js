...
const { IgApiClient } = '../src';
const { get } = 'request-promise'; // request is already declared as an dependency of the library

(async () => {
  const ig = new IgApiClient();
  ig.state.generateDevice('awaisrahman007');
  //ig.state.proxyUrl = process.env.IG_PROXY;
  const auth = await ig.account.login('awaisrahman007', 'Pakistan1!');
